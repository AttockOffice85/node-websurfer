...
export async function performHumanActions(page: Page, logger: Logger) {
  ...
  const selectRandomText = async () => {
    await page.evaluate(() => {
      ...
      const elements = Array.from(document.querySelectorAll("p, h1, h2, h3, h4, h5, h6, span")).filter((el) => !el.closest('button, a, [role="button"], [role="link"]'));
      ...
      if (elements.length > 0) {
        ...
        if (text.length > 0) {
          ...
          const textNodes = Array.from(randomElement.childNodes).filter((node) => node.nodeType === Node.TEXT_NODE && node.textContent!.trim().length > 0);
...
export async function likeRandomPosts(page: Page, count: number, logger: Logger): Promise<void> {
  ...
  while (likeButtons.length < count) {
    ...
    const newButtons = await page.$$(platformConfig.postLikeBtn);
...
export async function likeRandomPostsWithReactions(page: Page, count: number, logger: Logger): Promise<void> {
  ...
  try {
    ...
    while (likeButtons.length < count) {
      ...
      likeButtons = await page.$$(platformConfig.postLikeBtn);
...
export async function performLinkedInNetworkActions(page: Page, logger: Logger) {
  ...
  const handleConnectButtons = async () => {
    const connectButtons = (
      await Promise.all(
        allNetworkButtons.map(async (button) => {
  ...
  if (randomValue >= 20 && randomValue <= 70) {
    // handleConnectButtons on chance 50%
    await handleConnectButtons();
  }
  if (randomValue >= 40 && randomValue <= 60) {
    // handleInvitations on chance 20%
    await handleInvitations();
  }
  if (randomValue >= 5 && randomValue <= 20) {
    // handleSubscribeButtons on chance 15%
...
/**
 * Send friend requests to random profiles with error handling and rate limiting
 * @param page Playwright Page instance
 * @param requestCount Number of friend requests to send
 * @param maxRequests Maximum allowed requests (default: 20)
